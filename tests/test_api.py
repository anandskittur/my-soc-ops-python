...
class TestHomePage:
    ...
    def test_home_contains_start_screen(self, client: TestClient):
        response = client.get("/")
        assert "Soc Ops" in response.text or "SOC OPS" in response.text
        assert "START GAME" in response.text
        assert "How to play" in response.text or "HOW TO PLAY" in response.text
...
class TestStartGame:
    def test_start_returns_game_board(self, client: TestClient):
        ...
        assert any(b in response.text for b in ["BACK", "← Back", "&lt; BACK"])
